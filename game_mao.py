...
model_path = 'training_2'
# model = tf.keras.models.load_model(model_path + '/siamese_model.h5')
model = tf.keras.models.load_model(model_path + '/full_model.h5')
...
def netPredict(first, second):
    ...
    x_1 = np.array(bitifyFEN(beautifyFEN(first.fen()))).reshape(1,-1)
    x_2 = np.array(bitifyFEN(beautifyFEN(second.fen()))).reshape(1,-1)
    # toEval = [[x_1], [x_2]]
    # result = y.eval({x: toEval})
    result = model.predict([x_1,x_2])
...
def computerMove(board, depth=2):
    alpha = -1
    beta = 1
    v = -1
